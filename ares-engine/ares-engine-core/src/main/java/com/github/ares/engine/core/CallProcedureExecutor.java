...
public class CallProcedureExecutor extends AbstractBaseExecutor implements Serializable {
    ...
    public List<Serializable> evaluate(
            LogicalCallFunction callFunction,
            Map<String, CreateProcedureFunc> functions,
            PlParams plParams) {
        CreateProcedureFunc createProcedureFunc =
                functions.get(callFunction.getFuncName().toLowerCase());
        List<Serializable> args = new ArrayList<>();

        for (LogicalExpression expr : callFunction.getArgs()) {
            Serializable value =
                    executorManager.getExpressionExecutor().execute(expr.getExpr(), plParams);
            args.add(value);
        }
    ...
    public void execute(
            LogicalCallFunction callFunction,
            Map<String, CreateProcedureFunc> procedures,
            PlParams plParams) {
        // call procedure
        if (procedures.containsKey(callFunction.getFuncName().toLowerCase())) {
            ...
            if (result.size() != callFunction.getOutArgs().size()) {
                throw new AresException(
                        "The number of out arguments of the procedure '"
                                + callFunction.getFuncName()
                                + "' does not match");
            }
            int i = 0;

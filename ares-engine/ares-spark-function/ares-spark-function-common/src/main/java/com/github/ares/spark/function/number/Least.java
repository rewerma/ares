...
public class Least implements SparkFuncInterface {
    @Override
    ...
    public Object evaluate(List<Object> args) {
        ...
        if (isAllInt) {
            return args.stream().map(v -> ((Number) v).longValue()).min(Long::compare).get();
        }
        if (isDouble(args)) {
            return args.stream().map(v -> ((Number) v).doubleValue()).min(Double::compare).get();
        }
        return null;
    }

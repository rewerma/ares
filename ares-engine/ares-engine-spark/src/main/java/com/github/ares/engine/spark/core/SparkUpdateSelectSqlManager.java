...
public class SparkUpdateSelectSqlManager extends UpdateSelectSqlExecutor implements Serializable {
    ...
    public void execute(
        ...
        if (sparkExecutorManager.tryTransactionalSink(
                aresSink, resultDf, catalogTable, usSql.getSinkTable().getTableName())) {
            return;
        }
        ...
        if (executorManager
                .getSourceTables()
                .containsKey(usSql.getSinkTable().getTableName().toLowerCase())) {
            executorManager
                    .getReloadFunctionExecutor()
                    .reloadSourceTable(usSql.getSinkTable().getTableName());
        }
    }
}

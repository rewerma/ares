...
public class HintParser {
    ...
    public static Pair<List<SQLHint>, String> parseSelectHints(
            String sql, SqlBaseParser.QueryPrimaryDefaultContext queryPrimaryDefaultContext) {
        ...
        SqlBaseParser.SelectClauseContext selectClauseContext =
                regularQuerySpecificationContext.selectClause();
        if (!selectClauseContext.hints.isEmpty()) {
            for (int i = 0; i < selectClauseContext.hints.size(); i++) {
                ...
                for (SqlBaseParser.PrimaryExpressionContext primaryExpressionContext :
                        hintContext.hintStatement.parameters) {
    ...
    private static void filterHints(
            SqlBaseParser.SelectClauseContext selectClauseContext, StringBuilder selectSql) {
        for (ParseTree grandChild : selectClauseContext.children) {
            if (grandChild instanceof SqlBaseParser.IntoClauseContext
                    || grandChild instanceof SqlBaseParser.HintContext) {
                if ((grandChild instanceof SqlBaseParser.HintContext)
                        && ExecutionEngineType.engineType == EngineType.SPARK) {
                    String hint = getFullText(grandChild);
                    String hintLower = hint.toLowerCase();
                    if (hintLower.contains(INNER_HINT_MAPJOIN)
                            || hintLower.contains(INNER_HINT_BROADCAST)) {

...
public class PlCreateSourceTableVisitor {
    ...
    public LogicalOperation visitCreateSourceTable(
            PlSqlParser.Create_tableContext create_tableContext, Map<String, Object> withOptions) {
        String createTableName = create_tableContext.table_name().getText();
        if (sourceTables.containsKey(createTableName.toLowerCase())) {
            throw new ParseException(
                    String.format("Source table name exists: %s", createTableName));
        }
        ...
        List<PlSqlParser.Relational_propertyContext> columnsContext =
                create_tableContext.relational_table().relational_property();

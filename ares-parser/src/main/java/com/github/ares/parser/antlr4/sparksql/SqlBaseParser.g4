...
dmlStatementNoWith
    ...
    | DELETE FROM multipartIdentifier tableAlias
      (COMMA source=multipartIdentifier sourceAlias=tableAlias)?
      (COMMA LEFT_PAREN sourceQuery=query RIGHT_PAREN sourceAlias=tableAlias)? whereClause?                           #deleteFromTable
    | UPDATE multipartIdentifier tableAlias (COMMA source=multipartIdentifier sourceAlias=tableAlias)?
      (COMMA LEFT_PAREN sourceQuery=query RIGHT_PAREN sourceAlias=tableAlias)? setClause whereClause?                 #updateTable
...
selectClause
    : SELECT (hints+=hint)* setQuantifier? namedExpressionSeq intoClause?
    ;

intoClause
    : INTO expression (',' expression)*
    ;
...
matchedAction
    : DELETE
    | UPDATE SET ASTERISK
    | UPDATE SET assignmentList (WHERE updateCondition=booleanExpression)?
    ;

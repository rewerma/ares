...
public class AresFTPFileSystem extends FileSystem {
    public static final Log LOG = LogFactory.getLog(AresFTPFileSystem.class);
    ...
    private boolean dirExists(FTPClient client, Path dir) throws IOException {
        try {
            return getDirStatus(client, dir) != null;
        } catch (FileNotFoundException fnfe) {
            return false;
        }
    }
    ...
    private FileStatus getDirStatus(FTPClient client, Path file)  throws IOException {
        ...
        Path absolute = makeAbsolute(workDir, file);
        String pathName = absolute.toUri().getPath();
        String currentWorkDir = client.printWorkingDirectory();
        if (!client.changeWorkingDirectory(pathName)) {
            throw new FileNotFoundException("Directory " + pathName + " does not exist.");
        } else {

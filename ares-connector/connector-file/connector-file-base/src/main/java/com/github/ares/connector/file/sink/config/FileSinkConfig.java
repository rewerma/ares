...
public class FileSinkConfig extends BaseFileSinkConfig implements PartitionConfig {
    ...
    public FileSinkConfig(@NonNull Config config, @NonNull AresRowType aresRowTypeInfo) {
        super(config);
        checkArgument(!CollectionUtils.isEmpty(Arrays.asList(aresRowTypeInfo.getFieldNames())));
        ...
        if (CollectionUtils.isEmpty(this.sinkColumnList)) {
            ...
            this.sinkColumnList = new ArrayList<>(Arrays.asList(aresRowTypeInfo.getFieldNames()));
        }
        ...
        if (!config.hasPath(CommonOptions.HAS_TARGET_COLUMNS.key())
            ...
            //     throw new AresException("Sink columns number not equal to target columns
            // number");
            // }
        ...
        for (String sinkColumn : this.sinkColumnList) {
            ...
            throw new AresException(
                    String.format("Target column not found in sink columns: %s", sinkColumn));
        }
        ...
        for (String partitionField : this.partitionFieldList) {
            ...
            throw new AresException(
                    String.format(
                            "Target partition column not found in sink partitions: %s",
                            partitionField));
        }

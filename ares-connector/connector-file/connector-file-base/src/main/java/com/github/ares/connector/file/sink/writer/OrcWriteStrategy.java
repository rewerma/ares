...
public class OrcWriteStrategy extends AbstractWriteStrategy {
    ...
    public void write(@NonNull AresRow aresRow) {
        ...
        for (Integer index : sinkColumnsIndexInRow) {
            Object value = aresRow.getField(index);
    ...
    public static TypeDescription buildFieldWithRowType(AresDataType<?> type) {
        switch (type.getSqlType()) {
            ...
            case MAP:
                AresDataType<?> keyType = ((MapType<?, ?>) type).getKeyType();
                AresDataType<?> valueType = ((MapType<?, ?>) type).getValueType();
            ...
            case ROW:
                ...
                AresDataType<?>[] fieldTypes = ((AresRowType) type).getFieldTypes();
                for (int i = 0; i < fieldTypes.length; i++) {
                    struct.addField(
                            ((AresRowType) type).getFieldName(i).toLowerCase(),
    ...
    private TypeDescription buildSchemaWithRowType() {
        ...
        for (Integer i : sinkColumnsIndexInRow) {
            TypeDescription fieldType = buildFieldWithRowType(aresRowType.getFieldType(i));
            schema.addField(aresRowType.getFieldName(i).toLowerCase(), fieldType);
        }
        return schema;
    }
    ...
    private void setStructColumnVector(
        ...
        if (value instanceof AresRow) {
            AresRow aresRow = (AresRow) value;
            Object[] fields = aresRow.getFields();

...
public class OrcReadStrategy extends AbstractReadStrategy {
        ...
        try (Reader reader =
            ...
            for (int i = 0; i < aresRowType.getTotalFields(); i++) {
                TypeDescription typeDescription =
                        OrcWriteStrategy.buildFieldWithRowType(aresRowType.getFieldType(i));
                schema.addField(aresRowType.getFieldName(i), typeDescription);
            }
    ...
    public AresRowType getAresRowTypeInfo(String path) throws FileConnectorException {
        return getAresRowTypeInfoWithUserConfigRowType(path, null);
    }

    @Override
    public AresRowType getAresRowTypeInfoWithUserConfigRowType(
            String path, AresRowType configRowType) throws FileConnectorException {
        try (Reader reader =
                hadoopFileSystemProxy.doWithHadoopAuth(
                        ((configuration, userGroupInformation) -> {
                            OrcFile.ReaderOptions readerOptions =
    ...
    private AresDataType<?> orcDataType2AresDataType(
            TypeDescription typeDescription, AresDataType<?> configType) {
        switch (typeDescription.getCategory()) {
            ...
            case LIST:
                ...
                switch (aresDataType.getSqlType()) {
                    ...
                    default:
                        String errorMsg =
                                String.format(
                                        "Ares array type not supported this genericType [%s] yet",
                                        aresDataType);
                        throw new FileConnectorException(
            ...
            case MAP:
                ...
                if (configType instanceof MapType) {
                    ...
                    return new MapType<>(
                            orcDataType2AresDataType(keyType, null),
                            orcDataType2AresDataType(valueType, null));
                }
            case STRUCT:
                List<TypeDescription> children = typeDescription.getChildren();
                String[] fieldNames = typeDescription.getFieldNames().toArray(TYPE_ARRAY_STRING);
                AresDataType<?>[] fieldTypes = new AresDataType[children.size()];
                if (configType instanceof AresRowType) {
                    for (int i = 0; i < children.size(); i++) {
                        fieldTypes[i] =
                                orcDataType2AresDataType(
                                        children.get(i),
                                        ((AresRowType) configType).getFieldType(i));
                    }
                } else {
                    fieldTypes =
                            children.stream()
                                    .map(f -> orcDataType2AresDataType(f, null))
    ...
    private Object readColumn(
            ...
            @Nullable AresDataType<?> dataType,
            int rowNum) {
    ...
    private Object readDecimalVal(ColumnVector colVec, AresDataType<?> dataType, int rowNum) {
        ...
        if (!colVec.isNull[rowNum]) {
            ...
            for (int i = 0; i < fieldVec.length; i++) {
                if (dataType instanceof AresRowType) {
                    AresDataType<?> fieldType = ((AresRowType) dataType).getFieldType(i);
            ...
            structObj = new AresRow(fieldValues);
        }
        return structObj;
    }
